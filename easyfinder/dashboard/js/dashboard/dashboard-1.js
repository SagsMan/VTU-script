(function($) {
 ...
 var dzChartlist = function(){
	...
	document.getElementById("kyc-form")?.addEventListener("submit", async (e) => {
		...
		const submitButton = document.querySelector("#kyc-form button");
		const spinner = document.getElementById("spinner");
	
		const isValid = (bvn) => /^\d{11}$/.test(bvn.toString());
	
		if (!isValid(bvn) && !isValid(nin)) {
		  toastr.error("Please enter a valid 11-digit BVN or NIN.", "Validation Error", {
		...
		try {
		  const response = await fetch("./inc/index", {
			...
			body: JSON.stringify({ action: "kyc_verification", bvn, nin }),
		  });
		  ...
		  toastr.error(
			`An error occurred while submitting record: ${error}`,
			"Error",
			{
			  positionClass: "toast-top-right",
			  timeOut: 5000,
